...
const baseUrl = "https://yc-app-api.bioyx.cn/"
var orgNum = 'skyx' // 生科云店    
...
export default {
	...
	//验证手机号
	verifyMobileNumber:function(phoneNum) {
		let that = this;
...
export function upload_file(token, customerLeaveId,file) {
	...
	var	sign = md5(secret + 'username' + userName + 'timestamp' + time + 'token' + token +'orgNum'+orgNum+ secret)
		var headers = {};
		...
		uni.uploadFile({
			//supplier​/uploadBrandAuthorizationLetterImage?supplierId='+customerLeaveId,
			url: baseUrl + 'supplier/uploadBusinessLicenseImage?supplierId='+customerLeaveId,
			filePath:file,
...
export function upload_file1(token, customerLeaveId,file) {
	return new Promise((resolve, reject) => {
		var userName = uni.getStorageSync('userName')
	 	var token = uni.getStorageSync('token')
	...
	var	sign = md5(secret + 'username' + userName + 'timestamp' + time + 'token' + token +'orgNum'+orgNum+ secret)
		...
		uni.uploadFile({
			//supplier​/uploadBrandAuthorizationLetterImage?supplierId='+customerLeaveId,
			url: baseUrl + 'supplier/uploadBrandAuthorizationLetterImage?supplierId='+customerLeaveId,
			filePath:file,
			name:'brandAuthorizationLetterFiles',
			// files:file,
			header: header || {},
			success: (res) => {
				var datas = JSON.parse(res.data)
