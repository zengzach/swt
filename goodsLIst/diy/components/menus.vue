<template>
	<view v-show="!isSortType && menus.length" :class="bgStyle==1?'borderRadius15':''"
		...
		<view v-if="isMany">
			...
			<view class="dot acea-row row-center-wrapper" v-if="docConfig<2">
				<view class="dot-item" :class="{ 'line_dot-item': docConfig === 0,'': docConfig === 1}"
					:style="active==index?'background:'+ dotColor:''" v-for="(item,index) in menuList" :key="index"></view>
			</view>
		</view>
