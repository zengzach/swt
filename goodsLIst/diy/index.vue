<template>
	<view class="">
		<view v-if="pageShow&&isShow==1" class="page"
			...
			<view v-if="!errorNetwork" :style="colorStyle">
				...
				<view class="index skeleton" :style="{visibility: showSkeleton ? 'hidden' : 'visible'}">
					...
					<block v-for="(item, index) in styleConfig" :key="index">
						...
						<goodList ref="goodsList"  v-if="item.name == 'goodList'" :dataConfig="item" @detail="goDetail"
						...
						<scroll-view scroll-x="true" style="background: #fff;">
							<view class="sort-box" v-if="sortList.children && sortList.children.length">
								<view class="sort-item" v-for="(item, index) in sortList.children" :key="index"
									...
									<image :src="item.goodsPictureUrl" mode="" v-if="item.goodsPictureUrl"></image>
									<image src="/static/images/sort-img.png" mode="" v-else></image>
									<view class="txt">{{ $t(item.name) }}</view>
								</view>
							</view>
						</scroll-view>
						<view class="product-list" v-if="goodList.length">
							<view class="product-item" v-for="(item, index) in goodList" :key="index" @click="goGoodsDetail(item)">
								...
								<view class="info">
									<view class="title line1">{{ item.goodsTitle}}</view>
					...
					<view class="sort-product" v-if="isSortType == 1">
						<scroll-view scroll-x="true" style="background: #fff;">
							<view class="sort-box" v-if="sortList.children && sortList.children.length">
								<view class="sort-item" v-for="(item, index) in sortList.children" :key="index"
									@click="changeSort(item, index)" :class="{ on: curSort == index }">
									<image :src="item.imgUrl?item.imgUrl:'/static/sort-img.png'" mode=""></image>
									<view class="txt">{{ $t(item.name) }}</view>
								</view>
							</view>
						</scroll-view>
						<view class="product-list" v-if="goodList.length">
							<view class="product-item" v-for="(item, index) in goodList" :key="index" @click="goGoodsDetail(item)">
								...
								<view class="info">
									<view class="title line2">{{ item.goodsTitle }}</view>
									<view class="price-box">
										<text>{{$t(`￥`)}}</text>
										{{item.goodsPrice.promotionPrice !==0?item.goodsPrice.promotionPrice:item.goodsPrice.price}}
					...
					<!-- <pageFoot v-if="newid>-1" :diyId='newid'></pageFoot> -->
				</view>
			</view>
			<view v-else>
	...
	export default {
		...
		methods: {
			...
			bindHeighta(data) {
				...
				// // #endif
				this.sortMpTop = data.top;
				// console.log(data.top,"------------1--------------")
				// console.log(data.height,"---------2-----------------")
				// console.log(data.top + data.height,"---------1+2-----------------")
			},
			bindHeight(data) {
				uni.hideLoading();
			...
			getProductList(data) {
				...
				if (this.sortAll.length > 0) {
					this.sortAll.forEach((el, index) => {
						if (el.categoryId == data) {
							this.$set(this, 'sortList', el);
							this.sid = el.children.length ? el.children[0].categoryId : '';
						}
					});
					this.goodList = [];
					this.goodPage = 1;
			...
			getGoodsList() {
				...
				getProductslist({
					...
					// salesOrder: '',
					// news: 0,
					pageNum: this.goodPage,
					// limit: 10,
					// categoryId: this.sortList.categoryId
				}).then(res => {
			...
			diyData() {
				let that = this;
				// console.log(this.diyId,'this.diyId3333333333333333333333');
				getDiy(this.diyId).then(res => {
					uni.hideLoading()
					uni.setStorageSync('DIY_DATA', res.data.info)
					setTimeout(() => {
						this.isNodes++;
					}, 0);
					this.isShow = res.data.info.is_show
					...
					uni.setNavigationBarTitle({
						title: this.$t(res.data.info.title)
					});
					let temp = [];
					let lastArr = that.objToArr(res.data.info.value);
			...
			onsollBotton() {
				...
				this.$nextTick(() => {
				    ...
				    // 确认 goodsList 组件已经完成渲染后，再进行 $refs 属性调用
				    this.$refs.goodsList[0].onsollBottonTwo();
