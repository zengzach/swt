...
<script>
	import {integralBrandSortList} from "@/api/integral/integralIndexApi.js"
	export default {
		data() {
			return {
				barndArr: {}
		...
		methods: {
			getbrandSortList(){
				integralBrandSortList().then(res=>{
			...
			toList(item1){
				uni.navigateTo({
					url: "/integralPages/integralClassifyList?brandId=" + item1.id + '&brandName='+ item1.name +  '&categoryId=' + 307 + '&title=' + '全部分类'
				})
			},
		}
	}
